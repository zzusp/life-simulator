...
# 人生模拟器 (Life Simulator) Constitution
...
### II. 数据安全与合规
所有敏感数据必须通过Supabase加密存储；AI API密钥不得暴露给前端；所有用户输入必须经过内容审核(Moderator)；管理员操作必须记录审计日志；数据库访问必须使用行级安全策略(RLS)。

### III. 性能与可扩展性
所有AI调用必须实现缓存机制；数据库查询必须优化索引；前端组件必须支持懒加载；图片资源必须使用Next.js Image组件优化；API响应时间不得超过2秒。
